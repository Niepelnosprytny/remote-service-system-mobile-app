...
class CommentsPage extends ConsumerStatefulWidget {
  ...
  const CommentsPage(
      {super.key, required this.reportId, required this.reportTitle});
...
class _CommentsPageState extends ConsumerState<CommentsPage> {
  ...
  void initState() {
    super.initState();
    commentsSocket =
        WebSocketUtils("wss://sebastianinc.toadres.pl/api/websockets/chatroom");
        ...
        child: Column(
          children: [
                  ...
                  ? ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Padding(
                          ...
                          child: Align(
                            alignment: user?["id"] == comment["created_by"]
                                ? Alignment.centerRight
                                : Alignment.centerLeft,
                            child: Container(
                              constraints: BoxConstraints(maxWidth: 80.w),
                              padding: EdgeInsets.symmetric(
                                  horizontal: 0.5.h, vertical: 2.h),
                              decoration: BoxDecoration(
                                color: user?["id"] == comment["created_by"]
                                    ? const Color(0xFF7db3b4)
                                    : const Color(0xFFc77f89),
            ...
            Visibility(
                ...
                child: const SingleChildScrollView(child: FilesPicker())),
            Form(
              key: formKey,
              child: Padding(
                          ...
                          onTap: () {
                            if (formKey.currentState!.validate()) {
                              ...
                              Map<String, dynamic> notificationData = {
                                "content":
                                    "Nowy komentarz dla ${widget.reportTitle}",
                                "report_id": widget.reportId,
                                "users": ref.read(reportHandledByProvider)
                              };

                              ref.read(
                                  submitNotificationProvider(notificationData));

                              ref.read(submitCommentProvider(data));

                              FocusManager.instance.primaryFocus?.unfocus();

                              commentsSocket?.sendMessage(jsonEncode({
                                "message": 'init',
